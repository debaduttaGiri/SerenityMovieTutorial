namespace SereneMovieTutorial.Administration {
    ...
    export class PermissionEditor extends Serenity.DataGrid<PermissionItem, PermissionEditorOptions> {
        ...
        private getItemGrantRevokeClass(item: PermissionItem, grant: boolean): string {
            if (!item.IsGroup) {
        ...
        private getItemEffectiveClass(item: PermissionItem): string {
        ...
        private matchContains(item: PermissionItem): boolean {
            return Select2.util.stripDiacritics(item.Title || '').toLowerCase().indexOf(this.searchText) >= 0;
        }

        private getDescendants(item: PermissionItem, excludeGroups: boolean): PermissionItem[] {
