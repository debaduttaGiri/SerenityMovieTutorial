...
namespace SereneMovieTutorial.FuelManagement.Repositories
{
    ...
    public class FuelDataRepository
    {
        ...
        public decimal? GetLatestDieselRate(IDbConnection connection, DateTime date)
        {
            var fld = FuelDataRow.Fields;

            return connection.List<FuelDataRow>(q =>
            {
                q.Select(fld.RateLtr);
                q.Where(fld.InvoiceDate <= date);
