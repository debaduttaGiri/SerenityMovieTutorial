...
function titleize(str) {
  ...
  return result.split("_").filter((x) => x.length).map((x) => x.charAt(0).toUpperCase() + x.substring(1).toLowerCase()).join(" ");
}
...
function rangesToRows(ranges) {
  ...
  for (let i2 = 0; i2 < ranges.length; i2++) {
    for (let j = ranges[i2].fromRow; j <= ranges[i2].toRow; j++) {
      rows.push(j);
    }
  }
  return rows;
...
var RowSelectionModel = class _RowSelectionModel {
  ...
  handleKeyDown(e2) {
    ...
    selectedRows.sort(function(x, y2) {
      return x - y2;
    });
