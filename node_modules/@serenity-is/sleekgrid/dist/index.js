...
function initColumnProps(columns, defaults) {
  var usedIds = {};
  for (var i2 = 0; i2 < columns.length; i2++) {
    ...
    if (m2.minWidth && m2.width < m2.minWidth)
      m2.width = m2.minWidth;
    if (m2.maxWidth && m2.width > m2.maxWidth)
      m2.width = m2.maxWidth;
    if (m2.id == null || usedIds[m2.id]) {
      const prefix = m2.id != null && m2.id.length ? m2.id : m2.field != null ? m2.field : "col";
      var x2 = 0;
      while (usedIds[m2.id = prefix + (x2 == 0 ? "" : "_" + x2.toString())]) x2++;
    }
    usedIds[m2.id] = true;
    if (m2.name === void 0) {
      m2.name = titleize(m2.field ?? m2.id);
    }
  }
}
function titleize(str) {
  ...
  for (let i2 = 0; i2 < str.length; i2++) {
    const char = str[i2];
    const prevChar = i2 > 0 ? str[i2 - 1] : null;
    const nextChar = i2 < str.length - 1 ? str[i2 + 1] : null;
  ...
  return result.split("_").filter((x2) => x2.length).map((x2) => x2.charAt(0).toUpperCase() + x2.substring(1).toLowerCase()).join(" ");
}
...
// src/core/event.ts
var eventDataInitialized = false;
function addEventDataProp(name, isArgs) {
...
var EventDataWrapper = class {
  constructor(event, args) {
    this._isPropagationStopped = false;
    this._isImmediatePropagationStopped = false;
    this._isDefaultPrevented = false;
    this._returnValues = [];
    this._nativeEvent = event;
    this._args = args || {};
  ...
  isDefaultPrevented() {
    ...
    if (this._nativeEvent && typeof this._nativeEvent.isDefaultPrevented === "function")
      return this._nativeEvent.isDefaultPrevented();
    return false;
  }
  /***
...
var EventEmitter = class {
   ...
   */
  unsubscribe(fn) {
    for (var i2 = this._handlers.length - 1; i2 >= 0; i2--) {
      if (this._handlers[i2] === fn) {
        this._handlers.splice(i2, 1);
      }
    }
  }
  /***
   ...
   */
  notify(args, e2, scope) {
    const sed = new EventDataWrapper(e2, args);
    scope = scope || this;
    for (var i2 = 0; i2 < this._handlers.length && !(sed.isPropagationStopped() || sed.isImmediatePropagationStopped()); i2++) {
      const returnValue = this._handlers[i2].call(scope, sed, args);
...
var EventSubscriber = class {
  ...
  unsubscribe(event, handler) {
    var i2 = this._handlers.length;
    while (i2--) {
      if (this._handlers[i2].event === event && this._handlers[i2].handler === handler) {
        this._handlers.splice(i2, 1);
  ...
  unsubscribeAll() {
    var i2 = this._handlers.length;
    while (i2--) {
      this._handlers[i2].event.unsubscribe(this._handlers[i2].handler);
    }
    this._handlers = [];
    return this;
  }
...
function addCssClass(el, cls) {
  if (cls == null || !cls.length)
    return;
  if (cls.indexOf(" ") >= 0) {
    var arr = cls.split(" ").map((x2) => x2.trim()).filter((x2) => x2.length);
    for (var a2 of arr)
      el.classList.add(a2);
  } else
...
var maybeHtmlRegex = /<|>|&|"|'/;
function basicDOMSanitizer(dirtyHtml) {
  if (!dirtyHtml) {
    return "";
  }
...
function applyFormatterResultToCellNode(ctx, fmtResult, node, opt) {
  if (!opt?.contentOnly) {
...
function formatterContext(opt) {
  const gridOptions = opt?.grid?.getOptions?.();
  return {
    ...opt,
    ...
    sanitizer: opt?.sanitizer ?? gridOptions?.sanitizer ?? gridDefaults.sanitizer ?? // @ts-ignore
    (typeof DOMPurify !== "undefined" && typeof DOMPurify.sanitize == "function" ? DOMPurify.sanitize : basicDOMSanitizer),
    value: opt?.value
  };
}
...
var FrozenLayout = class {
  ...
  reorderViewColumns(viewCols, refs) {
    const pinnedStartCols = viewCols.filter((x2) => x2.frozen && x2.frozen !== "end");
    refs.config.pinnedStartCols = pinnedStartCols.length;
    if (pinnedStartCols.length > 0)
      return pinnedStartCols.concat(viewCols.filter((x2) => !x2.frozen));
    return null;
  }
  afterSetOptions(arg) {
...
function Draggable(options) {
  ...
  function executeDragCallbackWhenDefined(callback, evt, dragData2) {
    if (typeof callback === "function") {
      return callback(evt, dragData2);
    }
  }
  function destroy() {
    if (containerElement) {
  ...
  function userReleased(event) {
    ...
    if (dragStarted) {
      ...
      executeDragCallbackWhenDefined(onDragEnd, event, dragData);
      dragStarted = false;
    }
  }
  init();
...
function setupColumnResize({
  ...
  headerColsElements.forEach((hcols) => {
    headerColEls = headerColEls.concat(Array.from(hcols.children).filter((x2) => x2.classList.contains("slick-header-column")));
  });
  ...
  const docMouseUp = (e2) => {
    ...
    for (let j2 = 0; j2 < headerColEls.length; j2++) {
      const c2 = cols[j2];
      const newWidth = headerColEls[j2].offsetWidth;
  ...
  if (dist < 0) {
    ...
    for (j2 = cell; j2 >= 0; j2--) {
      ...
      if (c2.resizable) {
        ...
        if (x2 && c2.previousWidth + x2 < actualMinWidth) {
          x2 += c2.previousWidth - actualMinWidth;
          c2.width = actualMinWidth;
        } else {
          c2.width = c2.previousWidth + x2;
          x2 = 0;
        }
      }
    }
    if (forceFit) {
      x2 = -dist;
      for (j2 = cell + 1; j2 < cols.length; j2++) {
        c2 = cols[j2];
        if (c2.resizable) {
          if (x2 && c2.maxWidth && c2.maxWidth - c2.previousWidth < x2) {
            x2 -= c2.maxWidth - c2.previousWidth;
    ...
    for (j2 = cell; j2 >= 0; j2--) {
      c2 = cols[j2];
      if (c2.resizable) {
        if (x2 && c2.maxWidth && c2.maxWidth - (c2.previousWidth || 0) < x2) {
          x2 -= c2.maxWidth - (c2.previousWidth || 0);
          c2.width = c2.maxWidth;
        } else {
          c2.width = (c2.previousWidth || 0) + x2;
          x2 = 0;
          if (x2 == 0)
            break;
        }
...
function calcMinMaxPageXOnDragStart({ absoluteColMinWidth, cols, cell, forceFit, pageX }) {
  var shrinkLeewayOnRight = null, stretchLeewayOnRight = null, j2, c2;
  if (forceFit) {
...
function sortToDesiredOrderAndKeepRest(columns, idOrder) {
  ...
  function takeFrom(i3) {
    for (var j2 = i3; j2 < columns.length; j2++) {
      var c3 = columns[j2];
      if (i3 != j2 && orderById[c3.id] != null)
        break;
      result.push(c3);
...
function applyColumnWidths({ cols, cssColRulesL, cssColRulesR, opts, container, refs }) {
  let x2 = 0, w2, start = opts.rtl ? "right" : "left", end = opts.rtl ? "left" : "right", startVar = opts.rtl ? "--r" : "--l", endVar = opts.rtl ? "--l" : "--r", styles = container.style;
  for (let c2 = 0; c2 < cols.length; c2++) {
    ...
    w2 = cols[c2].width;
    let startVal = x2 + "px";
    let endVal = (c2 <= refs.pinnedStartLast ? refs.start.canvasWidth : c2 >= refs.pinnedEndFirst ? refs.end.canvasWidth : refs.main.canvasWidth) - x2 - w2 + "px";
    if (!cssColRulesL) {
...
var SleekGrid = class _SleekGrid {
  constructor(container, data, columns, options) {
    ...
    this.handleSelectedRangesChanged = (e2, ranges) => {
      ...
      for (var i2 = 0; i2 < ranges.length; i2++) {
        for (var j2 = ranges[i2].fromRow; j2 <= ranges[i2].toRow; j2++) {
          if (!hash[j2]) {
            this._selectedRows.push(j2);
            hash[j2] = /* @__PURE__ */ Object.create(null);
          }
          for (var k = ranges[i2].fromCell; k <= ranges[i2].toCell; k++) {
            if (this.canCellBeSelected(j2, k)) {
              const cid = cols[k].id;
              if (!isPollutingKey(j2) && !isPollutingKey(cid)) {
                hash[j2][cid] = this._options.selectedCellCssClass;
              }
            }
          }
        }
  ...
  updateBandCanvasWidths() {
    const oldCanvasWidths = this._mapBands((x2) => x2.canvasWidth);
    ;
  ...
  unbindAncestorScrollEvents() {
    if (this._boundAncestorScroll) {
      for (var x2 of this._boundAncestorScroll)
        this._off(x2, "scroll", bindThis(this).handleActiveCellPositionChange);
    }
    this._boundAncestorScroll = [];
  }
  ...
  createColumnFooters() {
    ...
    for (var i2 = 0; i2 < cols.length; i2++) {
      ...
      this._jQuery && this._jQuery(footerRowCell).data("column", m2);
      if (m2.footerCssClass)
        addCssClass(footerRowCell, m2.footerCssClass);
      else if (m2.cssClass)
        addCssClass(footerRowCell, m2.cssClass);
  ...
  createColumnHeaders() {
    ...
    for (let i2 = 0; i2 < cols.length; i2++) {
      ...
      if (typeof m2.nameFormat === "function") {
        const ctx = this.getFormatterContext(-1, i2);
        ctx.enableHtmlRendering = false;
        ctx.value = m2.name;
        const fmtResult = m2.nameFormat(ctx);
        applyFormatterResultToCellNode(ctx, fmtResult, nameSpan, contentOnly);
      } else
        nameSpan.textContent = m2.name ?? "";
        ...
        jsx(
          "div",
          {
            class: [
              ...
              m2.sortable && "slick-header-sortable"
            ],
            "data-id": m2.id,
            "data-c": i2,
            id: `${this._uid}${m2.id}`,
            title: m2.toolTip || "",
            style: { width: `${m2.width - this._headerColumnWidthDiff}px` },
            children: [
              nameSpan,
              m2.sortable && jsx("span", { class: "slick-sort-indicator", children: jsx("sub", {}) })
            ]
          }
        )
      );
      this._jQuery?.(header).data("column", m2);
      this._trigger(this.onHeaderCellRendered, { node: header, column: m2 });
      if (this._options.showHeaderRow) {
        ...
        this._jQuery?.(headerRowCell).data("column", m2);
        this._trigger(this.onHeaderRowCellRendered, { node: headerRowCell, column: m2 });
      }
    }
  ...
  setupColumnReorder() {
    if (typeof Sortable === "undefined")
      return;
    this.sortableColInstances?.forEach((x2) => x2.destroy());
    ...
    this.sortableColInstances = this._mapBands((band) => band.headerCols).map((x2) => (
      // @ts-ignore
      Sortable.create(x2, sortableOptions)
    ));
  }
  colResizing() {
  ...
  measureCellPaddingAndBorder() {
    ...
    el.remove();
    const r2 = this.getCanvasNode().appendChild(jsx("div", { class: "slick-row", children: [
  ...
  updateViewColLeftRight() {
    this._colLeft = [];
    this._colRight = [];
    var x2 = 0, r2, cols = this._cols, c2, l2 = cols.length;
    const { pinnedStartLast, pinnedEndFirst } = this._refs;
  ...
  setColumns(columns) {
    let initProps = true;
    if (columns && this._allCols && this._cols && columns.length === this._cols.length && this._allCols.length > this._cols.length && !columns.some((x2) => this._cols.indexOf(x2) < 0) && !this._cols.some((x2) => columns.indexOf(x2) < 0)) {
      columns = sortToDesiredOrderAndKeepRest(this._allCols, columns.map((x2) => x2.id));
      initProps = false;
    }
  ...
  queuePostProcessedRowForCleanup(cacheEntry, row) {
    ...
    for (var x2 in postProcessedRow) {
      var columnIdx = parseInt(x2, 10);
  ...
  updateRow(row) {
    ...
    for (var x2 in cacheEntry.cellNodesByColumnIdx) {
      var cell = parseInt(x2, 10);
  ...
  renderRows(range) {
    ...
    for (let row of rows) {
      ...
      if (cache.cellRenderContent.some((x2) => x2 instanceof Node))
        this.ensureCellNodesInRowsCache(row);
    }
  ...
  updateGrandTotals() {
    ...
    for (var m2 of cols) {
      if (m2.id != void 0) {
        const formatter = this.getTotalsFormatter(m2);
        if (!formatter)
          continue;
        ...
        const footerNode = this.getFooterRowColumn(m2.id);
  ...
  asyncPostProcessRows() {
    ...
    while (this._postProcessFromRow <= this._postProcessToRow) {
      ...
      for (var x2 in cacheEntry.cellNodesByColumnIdx) {
        var columnIdx = parseInt(x2, 10);
        var m2 = cols[columnIdx];
        var processedStatus = this._postProcessedRows[row][columnIdx];
        if (processedStatus !== "R") {
          if (m2.asyncPostRender || m2.asyncPostRenderCleanup) {
            var node = cacheEntry.cellNodesByColumnIdx[columnIdx];
            if (node) {
              m2.asyncPostRender && m2.asyncPostRender(node, row, this.getDataItem(row), m2, processedStatus === "C");
            }
          }
      ...
      if (removedRowHash) {
        for (columnId in removedRowHash) {
          if (!addedRowHash || removedRowHash[columnId] != addedRowHash[columnId]) {
            ...
            if (node) {
              const r2 = removedRowHash[columnId];
              removeCssClass(node, r2);
            }
          }
        }
      }
  ...
  getCellFromPoint(x2, y2) {
    var row = this.getRowFromPosition(y2);
    var cell = 0;
    var cols = this._cols;
    var w2 = 0;
    for (var i2 = 0; i2 < cols.length && w2 < x2; i2++) {
      w2 += cols[i2].width;
      cell++;
    }
    if (cell < 0) {
...
function CheckBoxFormatter(ctx) {
  ...
  i2.className = "slick-checkbox slick-edit-preclick" + (ctx.value ? " checked" : "");
  return i2;
}
...
// src/data/groupitemmetadataprovider.tsx
var GroupItemMetadataProvider = class _GroupItemMetadataProvider {
  constructor(opt) {
    this.pluginName = "GroupItemMetadataProvider";
    this.handleGridClick = (e2) => {
      let grid = e2?.grid ?? this.grid;
      if (!grid)
        return;
      var item = grid.getDataItem(e2.row);
      if (!item || !(item instanceof Group) || !this.options.toggleCssClass || !e2.target.classList.contains(this.options.toggleCssClass))
        return;
      e2.stopImmediatePropagation();
      e2.preventDefault();
    ...
    this.handleGridKeyDown = (e2) => {
      if (!this.options.enableExpandCollapse || e2.key !== " " && e2.key !== "-" && e2.key !== "+")
        return;
      let grid = e2?.grid ?? this.grid;
      if (!grid)
        return;
      ...
      e2.stopImmediatePropagation();
      e2.preventDefault();
      if (e2.key == "+" && !item.collapsed || e2.key == "-" && item.collapsed)
        return;
      var range = grid.getRenderedRange();
      grid.getData().setRefreshHints?.({
        ignoreDiffsBefore: range.top,
    ...
    this.groupCellPosition = () => {
      ...
      for (var idx = result.cell + 1; idx < cols.length; idx++) {
        var col2 = cols[idx];
        if (!this.options.hasSummaryType?.(col2) && !!col1?.frozen === !!col2?.frozen) {
          result.colspan++;
        } else
          break;
      }
    ...
    this.options.groupFormat ??= opt?.groupFormatter ? convertCompatFormatter(opt.groupFormatter) : (ctx) => _GroupItemMetadataProvider.defaultGroupFormat(ctx, this.options);
    this.options.totalsFormat ??= opt?.totalsFormatter ? convertCompatFormatter(opt.totalsFormatter) : (ctx) => _GroupItemMetadataProvider.defaultTotalsFormat(ctx, this.grid);
  }
  static {
    this.defaults = {
  ...
  static defaultGroupFormat(ctx, opt) {
    opt ??= _GroupItemMetadataProvider.defaults;
    let group = ctx.item;
    let fmtResultTitle;
    ...
    const titleSpan = jsx("span", { class: opt.groupTitleCssClass, "data-level": group.level.toString(), children: fmtResultTitle });
    applyFormatterResultToCellNode(ctx, fmtResultTitle, titleSpan, contentOnly2);
    return jsx(Fragment, { children: [
      jsx(
        "span",
        {
          class: opt.toggleCssClass + " " + (group.collapsed ? opt.toggleCollapsedCssClass : opt.toggleExpandedCssClass),
  ...
  static defaultTotalsFormat(ctx, grid) {
    ...
    if (!item.__groupTotals && item.totals)
      ctx.item = item = item.totals;
    grid = ctx.grid ?? grid;
    const formatter = grid ? grid.getTotalsFormatter(ctx.column) : ctx.column?.groupTotalsFormatter ? convertCompatFormatter(ctx.column?.groupTotalsFormatter) : ctx.column.groupTotalsFormat;
    if (formatter)
...
var AutoTooltips = class _AutoTooltips {
  constructor(options) {
    this.handleMouseEnter = (e2) => {
      var cell = this.grid.getCellFromEvent(e2);
      if (!cell)
        return;
    ...
    this.handleHeaderMouseEnter = (e2) => {
      var column = e2.column;
      if (column && !column.toolTip) {
        var node = e2.target.closest(".slick-header-column");
...
var RowMoveManager = class _RowMoveManager {
  constructor(options) {
  ...
  init(grid) {
    this.grid = grid;
    const boundThis = bindThis(this);
    this.handler.subscribe(grid.onDragInit, boundThis.handleDragInit).subscribe(grid.onDragStart, boundThis.handleDragStart).subscribe(grid.onDrag, boundThis.handleDrag).subscribe(grid.onDragEnd, boundThis.handleDragEnd);
  }
  destroy() {
  ...
  handleDragStart(e2, dd) {
    let cell = this.grid.getCellFromEvent(e2);
    ...
    const g2 = dd.guide = canvas.appendChild(document.createElement("div"));
    g2.style = `position: absolute; z-index: 9998; width: ${canvas.clientWidth}px; height: ${rowHeight}px; top: -1000px`;
    g2.className = "slick-row-move-guide";
    dd.insertBefore = -1;
  }
  handleDrag(e2, dd) {
    if (!this.dragging)
...
function rangesToRows(ranges) {
  ...
  for (let i2 = 0; i2 < ranges.length; i2++) {
    for (let j2 = ranges[i2].fromRow; j2 <= ranges[i2].toRow; j2++) {
      rows.push(j2);
    }
  }
  return rows;
}
var RowSelectionModel = class _RowSelectionModel {
  constructor(options) {
    this.handler = new EventSubscriber();
  ...
  handleKeyDown(e2) {
    ...
    if (!(activeRow && e2.shiftKey && !e2.ctrlKey && !e2.altKey && !e2.metaKey && (e2.which == 38 || e2.which == 40)))
      return;
    let selectedRows = this.getSelectedRows();
    selectedRows.sort(function(x2, y2) {
      return x2 - y2;
    });
  ...
  handleClick(e2) {
    ...
    if (!this.grid.getOptions().multiSelect || !e2.ctrlKey && !e2.shiftKey && !e2.metaKey) {
      return false;
    }
    let selection = rangesToRows(this.ranges);
    let idx = selection.indexOf(cell.row);
    if (idx === -1 && (e2.ctrlKey || e2.metaKey)) {
      selection.push(cell.row);
      this.grid.setActiveCell(cell.row, cell.cell);
    } else if (idx !== -1 && (e2.ctrlKey || e2.metaKey)) {
      selection = selection.filter((o2) => {
        return o2 !== cell.row;
      });
      this.grid.setActiveCell(cell.row, cell.cell);
    } else if (selection.length && e2.shiftKey) {
